...
class scrapeO:
	...
	def scrapeImages(self, start, stop):
		...
		for pid in (start, stop + 1):
			...
			if pid not in pidsList:
				...
				image = requests.get('http://i1.adis.ws//i//jpl//fp_{}_a?w=100&h=100'.format(pid))
				
				if 'File not found' in image.content :
					pass 

				else :
					bigImage = requests.get('http://i1.adis.ws//i//jpl//fp_{}_a?w=1000&h=1000'.format(pid))
					output = open(('{}.png'.format(pid)),"wb")
					output.write(bigImage.content)
					output.close()

					txt = open('FootPatrolPIDS.txt', 'a')
					txt.write(str(pid) + '\n')
					txt.close()
	...
	def checkPID(self, pid):
		...
		r = requests.get(('https://commerce.mesh.mx/stores/footpatrol/products/{}'.format(pid)), headers = headers)
